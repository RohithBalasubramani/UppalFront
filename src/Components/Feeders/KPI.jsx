...
const KPI = ({ data }) => {
  ...
  const currentPower =
    latestResampledData[latestResampledData.length - 1].kw_eb;
  const currentVoltage = data["recent data"].ln_avg_voltage;
  const kwToday = data["today"].kw_eb + data["today"].kw_dg;

  // Calculate peak current and peak voltage from the resampled data
  const peakCurrentData = data["results"].reduce((max, item) =>
    item.avg_current > max.avg_current ? item : max
  );
  const peakVoltageData = data["results"].reduce((max, item) =>
    item.ln_avg_voltage > max.ln_avg_voltage ? item : max
  );

  const peakCurrent = peakCurrentData.avg_current;
  const peakCurrentTime = new Date(peakCurrentData.timestamp);

  const peakVoltage = peakVoltageData.ln_avg_voltage;
