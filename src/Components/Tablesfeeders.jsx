...
// Define the formatColumnName function
...
const DataTable = ({
  ...
  // Blocked columns as per the reference code
  ...
  const filteredColumns = Object.keys(tablesData[0] || {}).filter(
    (column) => !blockedColumns.includes(column) && visibleColumns[column]
  );
  ...
  const filteredRows = sortedData.map((row) => {
    const roundedRow = {};
